...
const AvailableCropsPanel: FC<AvailableCropsPanelProps> = ({ onSelectCrop, selectedCropId, currency }) => {
  return (
    <Card className="shadow-lg">
      ...
      <CardContent className="space-y-3 max-h-[60vh] overflow-y-auto p-4">
        {CROPS_DATA.map((crop) => {
          ...
          const canAfford = currency >= crop.seedPrice;
          return (
            <Button
              key={crop.id}
              ...
              onClick={() => {
                if (canAfford || isSelected) { // Allow deselecting even if cannot afford anymore
                  onSelectCrop(crop.id)
                }
              }}
              disabled={!canAfford && !isSelected}
              title={!canAfford ? `Not enough gold (need ${crop.seedPrice})` : crop.name}
            >
              <div className="flex items-center space-x-3 w-full">
                ...
                <div className="flex-grow text-left">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Seed: <Coins className="inline w-3 h-3 mr-0.5" />{crop.seedPrice} | Grows in: {crop.growTime / 1000}s | Sells: <Coins className="inline w-3 h-3 mr-0.5" />{crop.sellPrice}
                  </p>
                </div>
